...
class ProductsList extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function isShow($item)
    {
		...
		if($item=="addto")
		{
			$show=str_replace("addtocart","",$show);
	...
	public function getAddToComparePostParams($product){
		$data = [];
		$data['product'] = $product->getId();
		if($this->getData('current_url'))
		{
			$data[\Magento\Framework\App\ActionInterface::PARAM_NAME_URL_ENCODED] = base64_encode($this->getData('current_url'));
		}
		return json_encode(['action' => $this->_compareProduct->getAddUrl(), 'data' => $data]);
	}
}
