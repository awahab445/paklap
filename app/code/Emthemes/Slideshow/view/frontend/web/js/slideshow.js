...
(function (factory) {
	...
	$.widget('custom.EmthemesSlideshow', {
		...
		_buildSlideShow: function(config){
			...
			if($(".emthemes-slideshow").hasClass("slideshow-loading")){
				slider.api.addEventListener(MSSliderEvent.WAITING , function(){
					$(".emthemes-slideshow").removeClass("slideshow-loading");
					$(".emthemes-slideshow").removeAttr("style");
				});
			}
