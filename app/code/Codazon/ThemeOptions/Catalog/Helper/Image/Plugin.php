    ...
    public function beforeInit($subject, $product, $imageId, $attributes = [])
    {
        if($this->_state->getAreaCode() == 'frontend'){
        	...
        	if($imageId == 'category_page_list'){
        		$attributes['aspect_ratio'] = (bool)$this->_helper->getConfig('general_section/category_view/keep_image_ratio');
//        		$attributes['width'] = $this->_helper->getConfig('general_section/category_view/image_width')/2;
//        		$attributes['height'] = $this->_helper->getConfig('general_section/category_view/image_height')/2;
